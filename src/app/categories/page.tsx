        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {[1, 2, 3, 4, 5, 6, 7, 8].map((n) => (
              <div key={n} className="h-44 rounded-3xl bg-white p-6 border border-slate-200 animate-pulse shadow-xs" />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {categoriesWithStats.map(({ category, count }) => (
              <Link
                ...
                className="surface-card surface-card-hover rounded-3xl p-6 flex flex-col justify-between group border border-slate-200 bg-white hover:border-brand-500/70 hover:shadow-md transition-all"
              >
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <div className="w-12 h-12 rounded-2xl bg-brand-50 group-hover:bg-brand-500 text-brand-600 group-hover:text-white flex items-center justify-center transition-colors shadow-2xs border border-brand-100/80">
                      <CategoryIcon nameOrIcon={category.icon || category.slug} className="w-6 h-6" />
                    </div>
                    <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-slate-100 text-slate-700 group-hover:bg-brand-50 group-hover:text-brand-600 transition-colors">
